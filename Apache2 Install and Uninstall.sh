...
echo
echo
read -p "Install/Check for Updates? [y/n]" answer
if [[ $answer = y ]] ; then
  echo
  clear
...
BLUE='\033[1;34m'
echo
read -p "Uninstall apache? before installing? (EVERYTHING WILL BE GONE FOREVER) [y/n]" answer
if [[ $answer = y ]] ; then
  echo
echo
sleep 1
...
printf "${RED}make sure to note your IP address. you will need it for port forwarding${NC}"
echo
...
echo
printf  "${RED}also also (haha) do not change the Document root.${NC}"
echo
printf  "${GREEN}<VirtualHost *:80>${NC}\n"
printf  "${GREEN} ServerAdmin (your admin email from the domain must be Vaild.)  ${NC}\n"
...
echo
echo
printf "Starting Aapache2"
clear
sudo service apache2 start
echo
printf "if there is nothing above me, then I worked!"
echo
printf "Lets do 1 last part, Enable the Firewall (will Add SSH (port 22) and Web Hosting (port 80)"
echo
echo
read -p "Do you want to turn the the Firewall? (HIGHLY RECOMMENDED) y/n: " firewall
echo
if [[ $firewall = y ]] ; then
  echo
  sudo ufw allow 22
  ...
  sudo ufw enable
  echo
  printf "done adding the firewall stuff"
  echo
  read -p "I recommend you restart because of the firewall. [y/n]: " shutdown
	if [[ $shutdown = y ]] ; then
		echo
		printf "Ok, ill restart when done!"
		echo
		echo
		...
		echo
		echo
		printf "Now for reboot.... ill do it in 30 seconds"
		sleep 15
		echo
		echo "15 seconds"
		echo
...
printf "we are fully working, i will die. if you ever want to run me again, just run the SH file!"
echo "so... GL,HF - Nolan of NRRINC Media"
echo
echo
